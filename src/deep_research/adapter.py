"""DeepResearchAdapter 实现

将 MultiTurnReactAgent 的输出适配为 LangGraph 可消费的消息序列。
"""
...
class DeepResearchAdapter(RunnableSerializable):
    ...
    def invoke(self, state: ResearchState, config: Optional[RunnableConfig] = None) -> Dict[str, Any]:
        ...
        while attempt < max(self.max_retries or 1, 1):
            attempt += 1
            try:
                input_payload = {
    ...
    def create_executor_graph(self, planner: Runnable, synthesizer: Runnable) -> Any:
        """创建执行图，由 DeepResearchNode 使用"""
        ...
        graph.add_node("deepresearch", self)
